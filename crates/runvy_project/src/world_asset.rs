...
use runvy_asset::{AudioAsset, Handle, TextureAsset};
use runvy_core::components::ui::{CanvasSpace, UiAssetFile, UiRenderer};
...
use runvy_core::glam::{IVec2, Quat, USizeVec2, Vec2, Vec3};
use runvy_core::ocs::{Object, ObjectComponentInfo};
...
pub fn load_world_with_runtime_registry(
    ...
    runtime_registry: &runvy_core::registry::RuntimeRegistry,
...
fn apply_parent_links(
    world: &mut World,
    spawned_ids: &[runvy_core::ocs::ObjectId],
...
impl WorldObjectAsset {
    ...
    fn from_object_with_parent_map(
        object: &Object,
        object_index: &HashMap<runvy_core::ocs::ObjectId, usize>,
    ) -> Self {
    ...
    pub fn into_object_with_runtime_registry(
        self,
        project_root: Option<&Path>,
        runtime_registry: Option<&runvy_core::registry::RuntimeRegistry>,
    ) -> Object {
        ...
        if let (Some(object_def_id), Some(registry)) = (object_id.as_ref(), runtime_registry) {
            let key = runvy_core::registry::ObjectDefKey::from(object_def_id.clone());
...
fn get_matches_type(kind: SerializedTypeKind, info: ObjectComponentInfo) -> bool {
    match kind {
        SerializedTypeKind::Component => {
            info.kind() == runvy_core::components::ComponentRuntimeKind::Component
        }
        SerializedTypeKind::Script => {
            info.kind() == runvy_core::components::ComponentRuntimeKind::Script
        }
    }
}
...
fn find_registered_type_metadata(
    registry: &runvy_core::registry::RuntimeRegistry,
    type_name: &str,
) -> Option<runvy_core::registry::TypeMetadata> {
...
fn infer_mesh_primitive(mesh: Handle<Mesh>) -> Option<MeshPrimitiveAsset> {
    ...
    let size = max - min;
    match hint {
        runvy_core::components::BuiltinMeshPrimitive::Cube => Some(MeshPrimitiveAsset::Cube {
            size: size.x.abs().max(size.y.abs()).max(size.z.abs()),
        }),
        runvy_core::components::BuiltinMeshPrimitive::Quad => Some(MeshPrimitiveAsset::Quad {
            width: size.x.abs(),
            height: size.y.abs(),
        }),
        runvy_core::components::BuiltinMeshPrimitive::Plane => Some(MeshPrimitiveAsset::Plane {
            width: size.x.abs(),
            depth: size.z.abs(),
        }),
        runvy_core::components::BuiltinMeshPrimitive::Pyramid => Some(MeshPrimitiveAsset::Pyramid {
            width: size.x.abs(),
            height: size.y.abs(),
            depth: size.z.abs(),
        }),
...
mod tests {
    ...
    use runvy_core::ocs::{Object, Script, ScriptContext};
    use runvy_core::registry::RuntimeRegistry;
    use std::fs;
...
"#;
        ...
        assert_eq!(
            world
                .borrow()
                .query::<runvy_core::components::Transform>()
                .len(),
            1
        );
    }
