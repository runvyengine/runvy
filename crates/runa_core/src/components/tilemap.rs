...
impl Tilemap {
    ...
    pub fn atlas_frame_count(&self) -> u32 {
        self.atlas.as_ref().map(TilemapAtlas::frame_count).unwrap_or(1)
    }

    /// UV rect of an atlas frame (clamped to the atlas grid), if one is set.
    pub fn atlas_uv(&self, frame: u32) -> Option<UvRect> {
        self.atlas.as_ref().map(|atlas| atlas.uv_rect_for_frame(frame))
    }
    ...
    pub fn set_tile(&mut self, layer_index: usize, world_x: i32, world_y: i32, tile: TileId) -> bool {
    ...
    pub fn tile_to_world(&self, tile_x: i32, tile_y: i32) -> Vec3 {
        let tile_size = self.world_tile_size();
        Vec3::new(tile_x as f32 * tile_size.x, tile_y as f32 * tile_size.y, 0.0)
    }
